...
def main():
    # Set up the page
    st.set_page_config(page_title="ArXiv Analysis Assistant", layout="wide")
    st.title("ArXiv Analysis Assistant")

    # Initialize chat history structure
    if "chats" not in st.session_state:
        st.session_state.chats = {
            1: {"title": "New Research", "messages": []}
        }
    ...
    with st.sidebar:
        # New chat button
        if st.button("New Research", key="new_chat_btn"):
            # Create a new chat
    ...
    st.caption("Created using LangChain and MCP - Jo Cloud AI Hub ")
